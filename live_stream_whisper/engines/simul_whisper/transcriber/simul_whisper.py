...
class PaddedAlignAttWhisper:
    def __init__(self, cfg: AlignAttConfig) -> None:
        ...
        logger.debug(f"mode name: {model_name},Linear:{cfg.if_ckpt_path} device:{self.model.device}")
        self.CIFLinear = None
        if cfg.if_ckpt_path :
        ...
        # 準備初始 tokens，包含 prompt
        sot_sequence = list(self.tokenizer.sot_sequence)
        
        # 如果有 prompt，將其編碼並插入到 sot 序列中
        if cfg.prompt and cfg.prompt.strip():
            prompt_tokens = self.tokenizer.encode(" " + cfg.prompt.strip())
            # 在語言標記之後插入 prompt tokens
            # sot_sequence 格式: [sot, lang, task, ...]
            insert_position = len(sot_sequence)
            sot_sequence = sot_sequence[:insert_position] + prompt_tokens + sot_sequence[insert_position:]
            logger.info(f"使用 prompt: '{cfg.prompt}'，token 數量: {len(prompt_tokens)}")
        ...
        self.sot_index = sot_sequence.index(self.tokenizer.sot)
    ...
    def fire_at_boundary(self, chunked_encoder_feature: torch.Tensor):
        ...
        alphas = torch.sigmoid(alphas)
            # 添加调试
        logger.debug(f"[CIF Debug] alphas shape: {alphas.shape}")
        logger.debug(f"[CIF Debug] alphas mean: {alphas.mean().item():.4f}")
        logger.debug(f"[CIF Debug] alphas sum: {alphas.sum().item():.4f}")
        ...
        logger.debug(f"[CIF Debug] integrate[-1]: {integrate[-1].item():.4f}")
        ...
        logger.debug(f"[CIF Debug] important_positions count: {important_positions.numel()}")
    ...
    def infer(self, segment, is_last=False):
        self.new_segment = True
        logger.debug(f"seg len {len(segment)}, segs len :{len(self.segments)}, times:{len(self.segments)*len(segment)/16000} second, is last {is_last} ")
        with torch.no_grad():
